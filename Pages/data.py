...
class Data:
    ...
    def col_names(self, table_name):
        return [row[0] for row in self.run_query(f"SHOW COLUMNS FROM {table_name};")]
        ...
        with st.echo():
            ...
            def examples(table_name):
                return pd.DataFrame(
                    ...
                    columns=self.col_names(table_name)
                )
    ...
    def menu(self):
        def _choices(ch, ops):
            for option in ops:
                if ch == option:
                    _query = eval(f"self._{option.lower()}()")
                    if st.button(option):
                        self.run_query(_query)
    ...
    def _update(self):
        table_name = st.selectbox('Table name:', [i[0] for i in self.run_query('SHOW TABLES;')])
        column_name = st.selectbox('Set Column:', self.col_names(table_name))
